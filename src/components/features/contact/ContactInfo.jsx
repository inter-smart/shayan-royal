    ...
    return (
        <div className="w-full h-auto 3xl:p-[30px] md:p-[20px_15px] p-[15px] bg-[#F5F9FF] flex max-md:flex-wrap">
            {items.map((item, index) => (
                <div key={index}
                    className="md:w-[calc(100%/4)] w-full flex flex-col justify-center relative z-0 md:first:w-[35%] [&:not(:last-child)]:max-md:mb-[20px] [&:not(:last-child)]:max-sm:mb-[15px] [&:not(:first-child)]:xl:pl-[30px] [&:not(:first-child)]:md:pl-[17px] max-md:before:hidden first:before:hidden [&:not(:last-child)]:before:content-[''] [&:not(:last-child)]:before:absolute [&:not(:last-child)]:before:z-1 [&:not(:last-child)]:before:right-0 [&:not(:last-child)]:before:top-0 [&:not(:last-child)]:before:w-[1px] [&:not(:last-child)]:before:h-full [&:not(:last-child)]:before:bg-[#dde0e5]"
                >
                    <Heading
                        as="h2"
