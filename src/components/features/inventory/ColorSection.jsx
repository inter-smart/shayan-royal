        ...
        <div className="flex flex-col lg:flex-row justify-between items-center gap-10">
          ...
          <div className="3xl:w-[500px] 2xl:w-[400px] xl:w-[300px] md:w-[250px] w-full flex justify-center">
            <div className="text-center">
              <h4 className="text-[18px] lg:text-[24px] font-semibold text-black mb-5 uppercase">Colors</h4>
              <ul className="flex lg:flex-col flex-wrap items-center">
                {colorVariants?.map((color, index) => {
                  const isActive = activeColor?.name === color?.name;
                  return (
                    <li key={index} className="relative lg:mb-4 max-lg:mr-3 cursor-pointer" onClick={() => setActiveColor(color)}>
                      <div
                        className={`2xl:w-[42px] w-[30px] 2xl:h-[42px] h-[30px] rounded-full flex items-center justify-center ${
                          isActive ? "border border-[#D2D5DA] shadow-[0_0_0_2px_white] bg-white" : ""
                        }`}
                        style={{
                          backgroundColor: isActive ? "#ffffff" : color?.color,
                        }}
                      >
                        {isActive && (
          ...
          <div className="relative 3xl:w-[calc(100%-500px)] 2xl:w-[calc(100%-400px)] xl:w-[calc(100%-300px)] md:w-[calc(100%-250px)] max-sm:max-w-[300px] w-full 3xl:pl-[200px] 2xl:pl-[100px] md:pl-[75px]">
            ...
            >
              ...
              <div className="absolute top-0 bottom-0 m-auto left-[-15%] flex items-center -z-10">
                <Image
                  src={activeColor?.image_url ? `${mediaUrl}${activeColor.image_url}` : "/images/whiteCar.png"}
                  alt="Car"
                  width={780}
                  height={400}
              ...
              {specs2?.map((item, index) => {
                ...
                const startAngle = -50;
                const endAngle = 50;
                const angleDeg = startAngle + (index * (endAngle - startAngle)) / (total - 1);
                ...
                return (
                  ...
                  >
                    <div className="2xl:w-6 sm:w-5 w-4 2xl:h-6 sm:h-5 h-4 mb-1">
                      <Image src={item?.icon} alt={item?.label} width={24} height={24} className="object-contain w-full" />
                    </div>
                    <span className="text-xs text-[#181818] font-medium">{item?.label}</span>
                  </div>
                );
              })}
            </div>
