...
export default function ResponsiveTabsWithSwiper({ specList = detailsData }) {
  ...
  // Calculate slidesPerView based on available tabs
  const calculateSlidesPerView = (availableCount) => {
    return {
      slidesPerView: Math.min(2, availableCount),
      breakpoints: {
        420: { slidesPerView: Math.min(3, availableCount) },
  ...
  const swiperConfig = calculateSlidesPerView(availableTabs.length);
    ...
    <section className="w-full px-4 pt-6 pb-3 bg-white overflow-hidden">
      <div className="container mx-auto">
        ...
        <div className="mb-6 relative">
          ...
          {/* Navigation Arrows - Only show if there are more tabs than can fit */}
                ...
                {originalIndex === 1 ? (
                  ...
                  <div className="flex flex-wrap">
                    {(specList?.[originalIndex] || []).map((item, idx) => (
                      <div key={idx} className="w-full xs:w-1/2 lg:w-1/4  border-l border-[#D9D9D9] px-[15px] 2xl:px-[30px]">
                        <div className="flex items-start justify-between text-sm mb-3 border-b border-dashed border-[#2E4C99] pb-[10px] 2xl:pb-[15px]">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-y-4 gap-x-1 lg:gap-x-2 text-[14px] text-[#1F1F1F] font-base1">
                    {specList?.[originalIndex] && specList[originalIndex].length > 0 ? (
                      specList[originalIndex].map((item, i) => (
                        <div
                          key={i}
