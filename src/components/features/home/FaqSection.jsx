...
export default function FaqSection({ faqs, title, description, image }) {
        ...
        <div className="flex flex-wrap m-[-10px] ">
          ...
          >
            <div className="xl:max-w-[85%] ">
              ...
              <Accordion type="single" collapsible defaultValue="item-0" className="w-full space-y-2">
                {faqs?.map((item, index) => (
                  <AccordionItem key={index} value={`item-${index}`} className="border-b border-[#D3D3D3]">
                    <AccordionTrigger
          ...
          >
            <div className="w-full max-w-[1040px] flex item-center justify-end ml-auto">
              <Image
                src={image ? `${process.env.NEXT_PUBLIC_API_BASE_URL}${image}` : "/images/faqCar.png"}
                alt={title ? title : "faqCar"}
                width={650}
                height={315}
