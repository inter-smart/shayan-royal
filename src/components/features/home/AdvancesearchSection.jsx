...
// Shared URL param parsers — used by both this component and InventorySection
...
export default function CarSearchForm() {
  ...
  // nuqs — single source of truth for filter state in the URL
  ...
  const form = useForm({
    ...
    values: queryParams, // form stays in sync with URL state
    mode: "onSubmit",
  });

  // Sync makeId (numeric) from URL name so model dropdown filters correctly on load
  useEffect(() => {
  ...
  const onSubmit = async (values) => {
    try {
      ...
      if (pathname === "/inventory") {
        // Already on inventory — update URL in place, reset to page 1
        setQueryParams({ ...newParams, page: null });
      } else {
        // Navigate to inventory with params in the URL
  ...
  const handleClear = () => {
    ...
    form.reset(Object.fromEntries(Object.keys(empty).map((k) => [k, ""])));
    setMakeId(null);

    if (pathname === "/") return; // on home page, just reset form locally

    setQueryParams({ ...empty, page: null });
  };
  ...
  // makeId holds the numeric ID (for filtering models client-side),
  // while the URL/form stores the human-readable name
  const models = makeId ? dropdownData?.data?.models?.filter((m) => m.make_id == makeId) : [];

  // Use name as value for options that go into the URL (make, model, carType)
  ...
  const menuLinkClass =
    "3xl:!text-[14px] 2xl:!text-[12px] md:!text-[10px] !text-[12px] text-black max-w-full min-h-[35px] lg:min-h-[40px] 2xl:min-h-[50px] 3xl:min-h-[60px] text-black uppercase font-normal placeholder:!text-black placeholder:font-normal !w-full px-[12px] border !border-[rgba(46,76,153,0.34)] bg-white rounded-[3px] xl:rounded-[3px] 2xl:rounded-[4px] 3xl:rounded-[5px] font-normal outline-none shadow-none focus:outline-none focus:ring-0 focus:border-[#CCCCCC] focus:shadow-none data-[state=open]:border-[#00095b] data-[state=open]:shadow-none [&>svg]:hidden relative after:absolute after:top-0 after:right-[15px] after:bottom-0 after:content-[''] after:w-[10px] after:h-[5px] after:w-[10px] 2xl:w-[15px] 2xl:h-[8px] after:3xl:w-[15px] after:3xl:h-[7px] after:[background-image:url('/images/selectArrow.png')] after:bg-no-repeat after:bg-center after:bg-contain after:m-auto";

  const contentClass =
    "3xl:text-[18px] 2xl:text-[16px] xl:text-[12px] md:text-[10px] text-[12px] bg-white border border-[#CCCCCC] rounded-md shadow-md font-normal uppercase placeholder:!text-black max-h-[210px] overflow-auto";

  const itemClass =
    "3xl:text-[14px] 2xl:text-[12px] xl:text-[10px] md:text-[10px] text-[12px]  3xl:py-[10px] 3xl:px-4 px-[7px] hover:bg-[#e4f0fe] focus:bg-[#e4f0fe] focus:text-black cursor-pointer font-normal !uppercase placeholder:!text-black transition-none duration-20";
                ...
                <FormItem>
                  <FormControl>
                    <SearchableSelect
                      ...
                      value={field.value}
                      onValueChange={(value) => {
                        field.onChange(value); // value is now the make name
